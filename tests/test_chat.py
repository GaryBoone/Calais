...
class TestProcessResponseChunk:
    """Test the _process_response_chunk method of the Chat class."""
    ...
    def test_process_response_chunk(self, mocker):
        ...
        chat = Chat(None, 1, 1, 1, 1)
        content, stop = chat._process_response_chunk(mock_chunk)
    ...
    def test_process_response_chunk_multiple(self, mocker):
        ...
        chat._process_response_chunk = mocker.MagicMock(
            side_effect=[
                ("Hello", False),
                (" World", False),
                ("!", True),
        ...
        for chunk in chunks:
            _, stop = chat._process_response_chunk(chunk)
            if stop:
                break

        calls = [mocker.call(chunk) for chunk in chunks]
        chat._process_response_chunk.assert_has_calls(calls)

        assert chat._process_response_chunk.call_count == len(chunks)
    ...
    def test_process_empty_content_chunk(self, mocker):
        ...
        chat._process_response_chunk = mocker.MagicMock(return_value=("", False))

        # Process the mock chunk with empty content
        content, stop = chat._process_response_chunk(mock_chunk_empty_content)
        ...
        chat._process_response_chunk.assert_called_once_with(mock_chunk_empty_content)
    ...
    def test_normal_content(self, chat_instance, make_mock_chunk):
        ...
        content, stop = chat_instance._process_response_chunk(chunk)
        assert content == "Hello"
        assert not stop
    ...
    def test_content_with_length_reason(self, chat_instance, make_mock_chunk):
        ...
        with pytest.raises(ChatError) as exc_info:
            chat_instance._process_response_chunk(chunk)
...
class TestCheckReturnedChunk:
    """Test the _check_returned_chunk method of the Chat class."""

    def test_check_returned_chunk_with_valid_chunk(
        self, chat_instance, make_mock_chunk
    ):
        """Test _check_returned_chunk with a valid ChatCompletionChunk."""
        ...
        assert chat_instance._check_returned_chunk(valid_chunk) == valid_chunk

    def test_check_returned_chunk_with_tuple(self, chat_instance):
        """Test _check_returned_chunk with a tuple input."""
        tuple_input = ("Error", "This is a tuple, not a ChatCompletionChunk")
        with pytest.raises(ChatError) as exc_info:
            chat_instance._check_returned_chunk(tuple_input)
    ...
    def test_check_returned_chunk_with_invalid_type(self, chat_instance):
        """Test _check_returned_chunk with an unexpected type."""
        invalid_input = ["This is", "not a valid input type"]
        with pytest.raises(ChatError) as exc_info:
            chat_instance._check_returned_chunk(invalid_input)
        assert "Received an unexpected chunk type" in str(exc_info.value)


class TestCallGptApi:
    """Test the _call_api method of the Chat class."""
    ...
    def test_normal_response_handling(self, chat_instance):
        ...
        with mock.patch.object(
            chat_instance, "_call_api", return_value=mock_chunks
        ), mock.patch.object(
            chat_instance, "_check_returned_chunk", side_effect=lambda x: x
        ), mock.patch.object(
            chat_instance,
            "_process_response_chunk",
            side_effect=lambda x: (x.choices[0].delta.content, False),
        ):
            response_text, empty_count = chat_instance._call_gpt_api([])
    ...
    def test_timeout_handling(self, chat_instance):
        ...
        with mock.patch.object(
            chat_instance, "_call_api", side_effect=Exception("Timeout reached")
        ):
            with pytest.raises(Exception) as exc_info:
                chat_instance._call_gpt_api([])
            assert "Timeout reached" in str(exc_info.value)


class TestChatResponseGeneration:
    """Test the _generate_response method of the Chat class."""
    ...
    @patch("calais.chat.Chat._call_gpt_api")
    ...
    def test_generate_response_with_too_many_empty_chunks(
        ...
        with pytest.raises(ChatError) as exc_info:
            chat_instance._generate_response([], False)

        assert "Failed to receive a response from OpenAI." in str(exc_info.value)
        # Ensure _call_gpt_api was called max_retries + 1 times (initial attempt + retries)
    ...
    def test_generate_response_with_valid_json(
        ...
        """
        Test _generate_response retries when the response is valid JSON.
        """
        mock_call_gpt_api.return_value = (
        ...
        resp = chat_instance._generate_response([], False)
    ...
    def test_generate_response_with_invalid_json(
        ...
        """
        Test _generate_response retries when the response is not valid JSON.
        """
        mock_call_gpt_api.return_value = (
            '{"type": "command"',
            0,
